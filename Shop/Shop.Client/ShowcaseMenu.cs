...
namespace Shop.Client
{
    class ShowcaseMenu
    {
        ...
        private void AddNewShowcase()
        {
            ...
            var response = _httpClient.PostAsync("http://localhost:21857/Store", httpContent).Result;
            var content = response.Content.ReadAsStringAsync().Result;
        }

        private void PrintShowcases()
        {
            var response = _httpClient.GetAsync("http://localhost:21857/Store").Result;
            var content = response.Content.ReadAsStringAsync().Result;
        ...
        private void DeleteShowcase()
        {
            ...
            var httpContent = new StringContent(jsonContent, Encoding.UTF8, "application/json");
            var response = _httpClient.PatchAsync("http://localhost:21857/Store", httpContent).Result;
            var content = response.Content.ReadAsStringAsync().Result;
            Console.WriteLine(content);
        }
        ...
        private void EditShowcase()
        {
            ...
            var jsonContent = JsonConvert.SerializeObject(showcase, Formatting.Indented);
            var httpContent = new StringContent(jsonContent, Encoding.UTF8, "application/json");
            var response = _httpClient.PutAsync("http://localhost:21857/Store", httpContent).Result;
            var content = response.Content.ReadAsStringAsync().Result;
            Console.WriteLine(content);
        }
    }
